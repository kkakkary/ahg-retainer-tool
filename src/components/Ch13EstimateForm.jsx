...
export default function Ch13EstimateForm({ savedFormData, onFormChange, onGenerated }) {
  ...
  function notifyChange(updates) {
    if (onFormChange) onFormChange({ clientName, totalDebt, fees, ...updates });
  }
  ...
  async function handlePreview() {
    ...
    try {
      const result = await window.electronAPI.generatePreview({ formData: buildPayload(), templateFile: 'ch13_estimate.docx' });
  ...
  function handleClear() {
    ...
    setClientName(''); setTotalDebt(''); setFees(initialFees);
    notifyChange({ clientName: '', totalDebt: '', fees: initialFees });
  ...
  return (
    ...
    <div className="w-[520px] flex-shrink-0 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
        <h2 className="text-base font-semibold text-slate-800">Chapter 13 Estimate — Central District</h2>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-3">
        <p className="text-sm font-semibold text-slate-800">Additional Fees <span className="text-gray-400 font-normal">(leave blank if not applicable)</span></p>
            ...
            <div key={key} className="flex items-center justify-between py-2 gap-4">
              ...
              <div className="relative w-32 flex-shrink-0">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg pl-6 pr-2 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-slate-400"
                />
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center justify-between py-2 border-t border-gray-200">
          ...
          <span className="text-sm text-gray-700 w-32 text-right pr-2">$1,995.00</span>
        </div>
        <div className="flex items-center justify-between pt-1 border-t border-gray-300">
          <span className="text-sm font-semibold text-slate-800">Total Estimate</span>
          <span className="text-sm font-bold text-slate-800 w-32 text-right pr-2">
            ${formatMoney(total)}
          </span>
        </div>
      </div>
